...
// Our initial note model
class Note {
  constructor(color, shape, midi, oldVal, svg) {
    this.color = color
    this.shape = shape
    this.midi = midi
    this.oldVal = oldVal
...
const buildTone = () => {
  ...
  for (let octave of octaves) {
    for (let pitch of pitches) {
      toneJS_vals.push(`${pitch}${octave}`)
    }
  }
  return toneJS_vals
}

// Note factory to compose our note values
const noteBuilder = () => {
  let noteContainer = []
  let iterator = 0
  const toneVals = buildTone()
  ...
  colors.forEach((color, i) => {
    shapes.forEach((shape, j) => {
      if (iterator < 128) {
        noteContainer.push(
          new Note(color, shape, iterator, toneVals[iterator], svgs[j])
        )
        iterator++
      }
    })
